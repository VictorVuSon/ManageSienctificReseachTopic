...
public class LuotDangKyDAO {
	...
	public boolean suaLuotDK(LuotDangKy luotDK) {
		...
		String query = "UPDATE luotdangky SET timeOpen = '"+luotDK.getTimeOpen()+"', timeClose = '"+luotDK.getTimeClose()+"', nam =?, idCap=? WHERE idLDK = ?";
		int check = 0;
		try {
			PreparedStatement pstm = db.connectDB().prepareStatement(query);
			
			pstm.setInt(1, luotDK.getNam());
			pstm.setInt(2, luotDK.getIdCap());
			pstm.setInt(3, luotDK.getIdLuotDangKy());
